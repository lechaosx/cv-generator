...
const { BASE_COLORS, DEFAULT_BASE_VALUES, DEFAULT_LINKS } = parseColorScheme();
export { BASE_COLORS, DEFAULT_BASE_VALUES, DEFAULT_LINKS };
...
export function initColors(): void {
	...
	Object.assign(colorLinks, BASE_LINKS);
...
function makeColorInput(k: string, isBase: boolean): HTMLInputElement {
...
function makeVariantControls(k: string): HTMLElement {
	...
	const checkReset = () => { resetBtn.style.opacity = colorLinks[k] === BASE_LINKS[k] ? '0' : ''; };
	checkReset();
	resetBtn.addEventListener('click', () => {
		if (colorLinks[k]) dots[colorLinks[k]]?.classList.remove('active');
		const defaultLink = BASE_LINKS[k];
		if (defaultLink) { colorLinks[k] = defaultLink; dots[defaultLink]?.classList.add('active'); input.classList.add('is-linked'); }
		else             { delete colorLinks[k]; input.classList.remove('is-linked'); }
		applyLinkedColor(k); persist(); checkReset();
	});
...
export function buildColorPanelContent(): void {
	...
	variants.append(panelLabel, makeVariantControls('panel-dark'), makeVariantControls('panel-light'));
	...
	for (let i = 0; i < DARK_COLORS.length; i++) {
		...
		variants.append(label, makeVariantControls(DARK_COLORS[i]!), makeVariantControls(LIGHT_COLORS[i]!));
	}
	colorPanel.appendChild(variants);
}

export function buildResetSnapshot(): { state: typeof state; themeColors: Record<string, string>; colorLinks: Record<string, string> } {
